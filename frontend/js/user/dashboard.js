...
async function cargarReportes(token, empresaId) {
  try {
    // ← FIX: era /api/reportes/empresa/:id (no existe)
    ...
    tbody.innerHTML = ultimos.map(r => {
      ...
      const badgeClass  = { pendiente:'pendiente', aprobado:'aprobado', rechazado:'rechazado', revision:'revision' }[estado] || 'pendiente';
      const estadoLabel = { pendiente:'Pendiente', aprobado:'Aprobado', rechazado:'Rechazado', revision:'En revisión' }[estado] || estado;
